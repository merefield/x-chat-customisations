  ...
  module ApiChannelControllerExtension
    def create
      if (
        ...
        if Category.exists?(name: name) || Group.exists?(name: group_name)
          raise Discourse::InvalidParameters.new(
                  "A Category or Group with the name #{name} already exists, choose a different name",
                )
        end
        ...
        GroupUser.create!(group_id: group.id, user_id: current_user.id, owner: true)
      end
      super
    end
  end
